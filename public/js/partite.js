function fetchAndProcessPartita(i, events) {
  ...
  const url = `/cercalikepartita?id=${id}`;
  fetch(url)
...
function onCercaLikePartitaJson(json, id, squadra1, squadra2, casa, ospite, competizione, orario, stadio, dataPartita) {
  ...
  bottone.addEventListener('click', function () {
    ...
    if (cuore.classList.contains('far')) {
      ...
      fetch(`/aggiungilikepartite?id=${id}&squadra1=${squadra1}&squadra2=${squadra2}&casa=${casa}&ospite=${ospite}&competizione=${competizione}&orario=${orario}&stadio=${stadio}&dataPartita=${dataPartita}`)
      ...
      cuore.classList.add('far');
      cuore.classList.add('animating');
      fetch(`rimuovilikepartite?id=${id}`)
        .then((response) => response.text())
        .then((data) => console.log(data))
        .catch((error) => console.log(error));
    }
